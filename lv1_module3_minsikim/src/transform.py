...
def make_T(R, t) -> np.ndarray:
    ...
    rrow, rcol = R.shape
    if (rrow != 3 or rcol != 3):
        raise ValueError(f"R이 3x3이 아닙니다. {R.shape}")
    if (len(t) != 3):
        raise ValueError(f"t의 크기가 3이 아닙니다. {len(t)}")
...
def inv_T(T) -> np.ndarray:
    ...
    trow, tcol = T.shape
    if (trow != 4 or tcol != 4):
        raise ValueError(f"T가 4x4가 아닙니다. {T.shape}")
    R = T[:3, :3] # 왼쪽 위 3x3 가져오기
    t = T[:3, 3] # 4번째열 위쪽 3개 가져오기
    R_inv = R.T
    t_inv = R_inv @ -t
    T_inv = np.eye(4)
    T_inv[:3, :3] = R_inv
